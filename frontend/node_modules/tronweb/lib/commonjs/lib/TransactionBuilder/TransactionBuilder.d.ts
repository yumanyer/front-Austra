...
export declare class TransactionBuilder {
    ...
    updateAccountPermissions(ownerAddress: string | undefined, ownerPermission: Permission, witnessPermission: Permission | undefined | null, activesPermissions: Permission | Permission[], options?: TransactionCommonOptions): Promise<Transaction<AccountPermissionUpdateContract>>;
    newTxID<T extends ContractParamter, U extends SignedTransaction<T> | Transaction<T>>(transaction: U, options?: {
        txLocal?: boolean;
    }): Promise<U>;
    alterTransaction<T extends Transaction>(transaction: T, options?: AlterTransactionOptions): Promise<T>;
    extendExpiration<T extends Transaction>(transaction: T, extension: number, options?: TxLocal): Promise<T>;
    addUpdateData<T extends Transaction>(transaction: T, data: string, dataFormat?: 'utf8' | 'hex', options?: TxLocal): Promise<T>;
}
export {};
