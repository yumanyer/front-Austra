    ...
    if (isObject(event.result)) {
        for (let i = 0; i < abi.length; i++) {
            ...
            if (obj.type == 'address' && obj.name in event.result)
                event.result[obj.name] = constants_js_1.ADDRESS_PREFIX + event.result[obj.name].substr(2).toLowerCase();
        }
    }
        ...
        event.result = event.result.reduce((obj, result, index) => {
            ...
            if (type == 'address')
                result = constants_js_1.ADDRESS_PREFIX + result.substr(2).toLowerCase();
            obj[name] = result;
            return obj;
        }, Object.create(null));
    }
