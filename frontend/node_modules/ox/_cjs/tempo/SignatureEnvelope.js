...
function deserialize_(value, multisigDepth) {
    ...
    if (typeId === serializedMultisigType) {
        ...
        for (const signature of signatures)
            if (Hex.size(signature) >
                MultisigConfig.maxOwnerSignatureBytes &&
                Hex.slice(signature, 0, 1) !==
    ...
    if (type === 'multisig') {
        ...
        const account = (() => {
            if (rest.account)
                return rest.account;
            if (initialConfig)
                return MultisigConfig.getAddress(initialConfig);
            return rest.account;
        })();
...
function sortMultisigApprovals(value) {
    ...
    const digest = MultisigConfig.getSignPayload('initialConfig' in value && value.initialConfig
...
class InvalidMultisigApprovalError extends Errors.BaseError {
    constructor({ reason }) {
