...
function getAddress(config) {
    assert(config);
    const hash = Hash.keccak256(Hex.concat(Hex.fromString(accountDomain), Hex.padLeft(config.salt ?? exports.zeroSalt, 32), Hex.fromNumber(config.threshold, { size: 1 }), Hex.fromNumber(config.owners.length, { size: 1 }), ...config.owners.flatMap((owner) => [
        owner.owner,
...
function getSignPayload(value) {
    ...
    const account = 'account' in value && value.account
        ? value.account
        : getAddress(value.initialConfig);
    return Hash.keccak256(Hex.concat(Hex.fromString(signatureDomain), Hex.from(payload), account, Hex.fromNumber(version, { size: 8 })));
}
