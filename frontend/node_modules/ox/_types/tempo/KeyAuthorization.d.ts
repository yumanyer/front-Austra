...
export type Signature<bigintType = bigint, numberType = number> = OneOf<SignatureEnvelope.Primitive<bigintType, numberType> | SignatureEnvelope.Multisig<bigintType, numberType>>;
/** RPC-formatted signature that can authorize an access key. */
export type SignatureRpc = OneOf<SignatureEnvelope.PrimitiveRpc | SignatureEnvelope.MultisigRpc>;
...
type SignatureValue = UnionPartialBy<Signature, 'prehash' | 'type'> | SignatureEnvelope.Secp256k1Flat | SignatureEnvelope.Serialized | SignatureEnvelope.from.MultisigFromInitialConfig;
 ...
 */
export declare function from<const authorization extends Input | Rpc, const signature extends SignatureValue | undefined = undefined>(authorization: authorization | KeyAuthorization, options?: from.Options<signature>): from.ReturnType<authorization, signature>;
export declare namespace from {
    type Options<signature extends SignatureValue | undefined = SignatureValue | undefined> = {
        /** The signature to attach to the Key Authorization. */
        signature?: signature | Signature | undefined;
    };
    type ReturnType<authorization extends KeyAuthorization | Input | Rpc = KeyAuthorization, signature extends SignatureValue | undefined = SignatureValue | undefined> = Compute<authorization extends Rpc ? Signed : TempoAddress.ResolveAddresses<authorization & (signature extends SignatureValue ? {
        signature: Extract<SignatureEnvelope.from.ReturnValue<signature>, Signature>;
    } : {})>>;
