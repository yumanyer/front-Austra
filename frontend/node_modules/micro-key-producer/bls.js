...
exports._TEST = exports.EIP2335Keystore = exports.EIP2334_KEY_TYPES = void 0;
...
function ikmToLamportSK(ikm, salt) {
    const okm = (0, hkdf_1.hkdf)(sha2_1.sha256, ikm, salt, undefined, 32 * 255);
...
function parentSKToLamportPK(parentSK, index) {
    parentSK = (0, utils_1.ensureBytes)('parentSK', parentSK);
    assertUint32(index);
    ...
    const lamportPK = lamport0.concat(lamport1).map((part) => (0, sha2_1.sha256)(part));
    return (0, sha2_1.sha256)((0, utils_2.concatBytes)(...lamportPK));
}
/**
...
function hkdfModR(ikm, keyInfo = new Uint8Array()) {
    ikm = (0, utils_1.ensureBytes)('IKM', ikm);
    keyInfo = (0, utils_1.ensureBytes)('key information', keyInfo);
    let salt = (0, utils_2.utf8ToBytes)('BLS-SIG-KEYGEN-SALT-');
    let SK = 0n;
    const input = (0, utils_2.concatBytes)(ikm, Uint8Array.from([0x00]));
    const label = (0, utils_2.concatBytes)(keyInfo, Uint8Array.from([0x00, 0x30]));
    while (SK === 0n) {
        salt = (0, sha2_1.sha256)(salt);
        const okm = (0, hkdf_1.hkdf)(sha2_1.sha256, input, salt, label, 48);
        SK = Fr.create(os2ip(okm));
    }
    return (0, utils_1.numberToBytesBE)(SK, 32);
}
...
function deriveEIP2334SigningKey(withdrawalKey, index = 0) {
    withdrawalKey = (0, utils_1.ensureBytes)('withdrawal key', withdrawalKey, 32);
    assertUint32(index);
...
function UUIDv4(buf) {
    ...
    return parts.map(utils_2.bytesToHex).join('-');
}
...
function deriveEIP2335Key(password, salt, kdf) {
    const pass = (0, utils_2.utf8ToBytes)(normalizePassword(password));
    if (kdf === 'scrypt') {
        ...
        return (0, pbkdf2_1.pbkdf2)(sha2_1.sha256, pass, salt, { c, dkLen });
    }
    else {
...
function decryptEIP2335Keystore(store, password) {
    ...
    const ciphertext = (0, utils_2.hexToBytes)(c.cipher.message);
    const salt = (0, utils_2.hexToBytes)(c.kdf.params.salt);
    const iv = (0, utils_2.hexToBytes)(c.cipher.params.iv);
    const key = deriveEIP2335Key(password, salt, c.kdf.function);
    // verify checksum
    const checksum = (0, utils_2.bytesToHex)((0, sha2_1.sha256)((0, utils_2.concatBytes)(key.subarray(16, 32), ciphertext)));
    ...
    if (store.pubkey !== undefined) {
        const publicKey = (0, utils_2.bytesToHex)(getPublicKey(secret));
...
class EIP2335Keystore {
    ...
    description = '', pubkey) {
        ...
        const checksum = (0, utils_2.bytesToHex)((0, sha2_1.sha256)((0, utils_2.concatBytes)(key.subarray(16), ciphertext)));
        const res = {
            version: 4,
            description,
            path,
            uuid: UUIDv4(uuid),
            crypto: {
                kdf: { function: kdf, params: { ...KDFS[kdf], salt: (0, utils_2.bytesToHex)(salt) }, message: '' },
                checksum: { function: 'sha256', params: {}, message: checksum },
                cipher: {
                    function: 'aes-128-ctr',
                    params: { iv: (0, utils_2.bytesToHex)(iv) },
                    message: (0, utils_2.bytesToHex)(ciphertext),
                },
            },
        };
        if (pubkey !== undefined)
            res.pubkey = (0, utils_2.bytesToHex)((0, utils_1.ensureBytes)('public key', pubkey));
        return res;
    }
    /**
