...
import { getAddress as getAddress2, toHex as toHex6, isHex } from "viem";
...
import { buildSignatureBytes } from "@tetherto/wdk-safe-protocol-kit";
...
import { EthSafeSignature, buildSignatureBytes as buildSignatureBytes2 } from "@tetherto/wdk-safe-protocol-kit";
...
function zkSyncCreate2Address(from, safeVersion, salt, input) {
  ...
  const addressBytes = keccak256(
    concat2([ZKSYNC_CREATE2_PREFIX, pad2(asHex(from)), salt, bytecodeHash, inputHash])
  ).slice(26);
...
function encodeSetupCallDataSync(safeAccountConfig, safeVersion) {
  ...
  if (major === 1 && minor === 0) {
    const setupData = encodeFunctionData3({
      abi: [
        {
          inputs: [
...
var Safe4337Pack = class _Safe4337Pack extends RelayKitBasePack {
  ...
  static predictSafeAddress({
    ...
    if (isBatch) {
      ...
      deploymentData = encodeFunctionData3({
        abi: ABI,
        functionName: "multiSend",
        args: [encodeMultiSendData2(setupTransactions)]
      });
    ...
    const salt = keccak256(concat2([initializerHash, encodedNonce]));
    const proxyCreationCode = getProxyCreationCode(chainIdBigInt);
    const checksummedSingletonAddress = getAddress2(singletonAddress);
    ...
    if (isZkSyncChain(chainId)) {
      ...
      return getAddress2(proxyAddress);
    }
    const initCode = concat2([proxyCreationCode, asHex(input)]);
    const hash = keccak256(
      concat2(["0xff", factoryAddress, salt, keccak256(initCode)])
    );
