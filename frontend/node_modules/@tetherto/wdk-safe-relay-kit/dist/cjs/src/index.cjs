...
var import_wdk_safe_protocol_kit = require("@tetherto/wdk-safe-protocol-kit");
...
var GelatoRelayPack = class extends RelayKitBasePack {
  ...
  async createPaymentToGelato(gas, options) {
    ...
    const transferToGelato = (0, import_wdk_safe_protocol_kit.createERC20TokenTransferTransaction)(
      gasToken,
      gelatoAddress,
      paymentToGelato
    );
  ...
  async createTransaction({
    ...
    const gasToken = options.gasToken ?? ZERO_ADDRESS;
    const isGasTokenCompatible = await (0, import_wdk_safe_protocol_kit.isGasTokenCompatibleWithHandlePayment)(
      gasToken,
      this.protocolKit
    );
  ...
  async createTransactionWithHandlePayment({
    ...
    const gasPrice = "1";
    const safeTxGas = await (0, import_wdk_safe_protocol_kit.estimateSafeTxGas)(this.protocolKit, transactionToEstimateGas);
    const gasToken = options.gasToken ?? ZERO_ADDRESS;
    ...
    const baseGas = await (0, import_wdk_safe_protocol_kit.estimateTxBaseGas)(this.protocolKit, transactionToEstimateGas);
    const safeDeploymentGasCost = await (0, import_wdk_safe_protocol_kit.estimateSafeDeploymentGas)(this.protocolKit);
...
var import_wdk_safe_protocol_kit5 = __toESM(require("@tetherto/wdk-safe-protocol-kit"));
...
var import_wdk_safe_protocol_kit2 = require("@tetherto/wdk-safe-protocol-kit");
var BaseSafeOperation = class {
  ...
  encodedSignatures() {
    return (0, import_wdk_safe_protocol_kit2.buildSignatureBytes)(Array.from(this.signatures.values()));
  }
  getUserOperation() {
    return {
      ...this.userOperation,
...
var import_wdk_safe_protocol_kit4 = require("@tetherto/wdk-safe-protocol-kit");
...
var import_wdk_safe_protocol_kit3 = require("@tetherto/wdk-safe-protocol-kit");
...
function getDummySignature(signer, threshold) {
  ...
  for (let i = 0; i < threshold; i++) {
    ...
    signatures.push(new import_wdk_safe_protocol_kit3.EthSafeSignature(signer, passkeySignature, isContractSignature));
  }
  return (0, import_viem4.encodePacked)(["uint48", "uint48", "bytes"], [0, 0, (0, import_wdk_safe_protocol_kit3.buildSignatureBytes)(signatures)]);
}
...
var USDT_ON_MAINNET = "0xdAC17F958D2ee523a2206206994597C13D831ec7";
...
function encodeMultiSendCallData(transactions) {
  return (0, import_viem7.encodeFunctionData)({
    ...
    args: [
      (0, import_wdk_safe_protocol_kit4.encodeMultiSendData)(
...
var EMPTY_DATA = "0x";
function asHex(hex) {
  return (0, import_viem9.isHex)(hex) ? hex : `0x${hex}`;
}
var EQ_OR_GT_1_4_1 = ">=1.4.1";
var SAFE_PROXY_CREATION_CODES = {
  latest: "",
  zkSync: {
    latest: ""
  }
};
...
function zkSyncCreate2Address(from, safeVersion, salt, input) {
  ...
  const inputHash = (0, import_viem10.keccak256)(input);
  const addressBytes = (0, import_viem10.keccak256)(
    (0, import_viem10.concat)([ZKSYNC_CREATE2_PREFIX, (0, import_viem10.pad)(asHex(from)), salt, bytecodeHash, inputHash])
  ).slice(26);
  return `0x${addressBytes}`;
}
function getSafeDeploymentInfo(chainId, safeVersion) {
  const singletonDeployment = (0, import_safe_deployments.getSafeL2SingletonDeployment)({
    version: safeVersion,
    released: true
  });
  const factoryDeployment = (0, import_safe_deployments.getProxyFactoryDeployment)({
    version: safeVersion,
    released: true
  });
...
function isZkSyncChain(chainId) {
  const ZKSYNC_CHAIN_IDS = /* @__PURE__ */ new Set([
    324,
    // zkSync Era mainnet
    300,
    // zkSync Era testnet
    ...
    // zkSync Era localnet
    232
    // zkSync Era internal testnet
  ]);
  return ZKSYNC_CHAIN_IDS.has(Number(chainId));
}
function getProxyCreationCode(chainId) {
  if (chainId && isZkSyncChain(chainId)) {
    return SAFE_PROXY_CREATION_CODES.zkSync.latest;
  }
  return SAFE_PROXY_CREATION_CODES.latest;
}
function encodeSetupCallDataSync(safeAccountConfig, safeVersion) {
  const {
    owners,
    threshold,
    to = import_viem10.zeroAddress,
    data = EMPTY_DATA,
    fallbackHandler = import_viem10.zeroAddress,
  ...
  if (major === 1 && minor === 0) {
    const setupData = (0, import_viem10.encodeFunctionData)({
      abi: [
        {
          inputs: [
...
var USDT_ON_MAINNET2 = "0xdAC17F958D2ee523a2206206994597C13D831ec7";
var Safe4337Pack = class _Safe4337Pack extends RelayKitBasePack {
  ...
  constructor({
    ...
    if (onchainAnalytics?.project) {
      ...
      this.#onchainIdentifier = (0, import_wdk_safe_protocol_kit5.generateOnChainIdentifier)({
        project,
        platform,
        tool: "relay-kit",
  ...
  static async init(initOptions) {
    ...
    if ("safeAddress" in options) {
      protocolKit = await import_wdk_safe_protocol_kit5.default.init({
        provider,
        signer,
      ...
      const isApproveTransactionRequired = !!paymasterOptions && !paymasterOptions.isSponsored && !!paymasterOptions.paymasterTokenAddress && !paymasterOptions.skipApproveTransaction;
      if (isApproveTransactionRequired) {
        const { paymasterAddress, amountToApprove = MAX_ERC20_AMOUNT_TO_APPROVE } = paymasterOptions;
        if (paymasterOptions.paymasterTokenAddress.toLowerCase() === USDT_ON_MAINNET2.toLowerCase()) {
          const predictedSafeAddress = await (0, import_wdk_safe_protocol_kit5.predictSafeAddress)({
            safeProvider: await import_wdk_safe_protocol_kit5.SafeProvider.init({ provider, signer, safeVersion }),
          ...
          const tempSafeProvider = await import_wdk_safe_protocol_kit5.SafeProvider.init({ provider, signer, safeVersion });
          const currentAllowance = await tempSafeProvider.readContract({
            address: paymasterOptions.paymasterTokenAddress,
            abi: ABI,
            functionName: "allowance",
            args: [predictedSafeAddress, paymasterAddress]
          });
      ...
      const safeProvider = await import_wdk_safe_protocol_kit5.SafeProvider.init({ provider, signer, safeVersion });
      ...
      if (isBatch) {
        const multiSendContract = await (0, import_wdk_safe_protocol_kit5.getMultiSendContract)({
          safeProvider,
          safeVersion,
          deploymentType: options.deploymentType || void 0
        });
        const batchData = (0, import_viem10.encodeFunctionData)({
          abi: ABI,
          functionName: "multiSend",
          args: [(0, import_wdk_safe_protocol_kit5.encodeMultiSendData)(setupTransactions)]
        });
  ...
  async getEstimateFee({
    ...
    if (estimateUserOperationGas) {
      if (feeEstimator.defaultVerificationGasLimitOverhead != null && estimateUserOperationGas.verificationGasLimit != null) {
        estimateUserOperationGas.verificationGasLimit = (BigInt(estimateUserOperationGas.verificationGasLimit) + BigInt(threshold) * feeEstimator.defaultVerificationGasLimitOverhead).toString();
      }
    ...
    if (postEstimationData) {
      if (feeEstimator.defaultVerificationGasLimitOverhead != null && postEstimationData.verificationGasLimit != null) {
        postEstimationData.verificationGasLimit = (BigInt(postEstimationData.verificationGasLimit) + BigInt(threshold) * feeEstimator.defaultVerificationGasLimitOverhead).toString();
      }
  ...
  async createTransaction({
    ...
    const paymasterOptions = this.#paymasterOptions ? { ...this.#paymasterOptions } : void 0;
    if (paymasterOptions) {
      if (isSponsored) {
        ;
        paymasterOptions.isSponsored = true;
        paymasterOptions.sponsorshipPolicyId = sponsorshipPolicyId;
        delete paymasterOptions.paymasterTokenAddress;
        delete paymasterOptions.amountToApprove;
    ...
    const userOperation = await createUserOperation(this.protocolKit, transactions, {
      entryPoint: this.#ENTRYPOINT_ADDRESS,
      paymasterOptions,
      amountToApprove: isSponsored ? void 0 : amountToApprove,
      customNonce
    });
  ...
  #toSafeOperation(safeOperationResponse) {
    ...
    if (safeOperationResponse.confirmations) {
      safeOperationResponse.confirmations.forEach((confirmation) => {
        safeOperation.addSignature(new import_wdk_safe_protocol_kit5.EthSafeSignature(confirmation.owner, confirmation.signature));
      });
    }
    return safeOperation;
  }
  ...
  async signSafeOperation(safeOperation, signingMethod = import_types_kit3.SigningMethod.ETH_SIGN_TYPED_DATA_V4) {
    ...
    if (isPasskeySigner) {
      ...
      if (!isSafeDeployed) {
        ...
        safeSignature = new import_wdk_safe_protocol_kit5.EthSafeSignature(
      ...
      if ([
        ...
        safeSignature = new import_wdk_safe_protocol_kit5.EthSafeSignature(signerAddress2, signature);
      } else {
        const safeOpHash = safeOp.getHash();
        safeSignature = await this.protocolKit.signHash(safeOpHash);
      }
  ...
  static predictSafeAddress({
    ...
    const isBatch = setupTransactions.length > 1;
    if (isBatch) {
      const multiSendDeployment = (0, import_safe_deployments.getMultiSendDeployment)({
        version: safeVersion,
        released: true
      });
      ...
      deploymentData = (0, import_viem10.encodeFunctionData)({
        abi: ABI,
        functionName: "multiSend",
        args: [(0, import_wdk_safe_protocol_kit5.encodeMultiSendData)(setupTransactions)]
      });
    ...
    const initializerHash = (0, import_viem10.keccak256)(initializer);
    const encodedNonce = (0, import_viem10.encodeAbiParameters)((0, import_viem10.parseAbiParameters)("uint256"), [BigInt(saltNonce)]);
    const salt = (0, import_viem10.keccak256)((0, import_viem10.concat)([initializerHash, encodedNonce]));
    const proxyCreationCode = getProxyCreationCode(chainIdBigInt);
    const checksummedSingletonAddress = (0, import_viem9.getAddress)(singletonAddress);
    const input = (0, import_viem10.encodeAbiParameters)((0, import_viem10.parseAbiParameters)("address"), [checksummedSingletonAddress]);
    if (isZkSyncChain(chainId)) {
      const proxyAddress = zkSyncCreate2Address(factoryAddress, safeVersion, salt, asHex(input));
      return (0, import_viem9.getAddress)(proxyAddress);
    }
    const initCode = (0, import_viem10.concat)([proxyCreationCode, asHex(input)]);
    const hash = (0, import_viem10.keccak256)(
      (0, import_viem10.concat)(["0xff", factoryAddress, salt, (0, import_viem10.keccak256)(initCode)])
    );
    const address = (0, import_viem10.slice)(hash, 12);
    return (0, import_viem9.getAddress)(address);
  }
};
