...
async function serializeTempo(transaction, sig) {
    ...
    const hasSenderSignature = typeof signature_provided !== 'undefined' ||
        Boolean(transaction.multisig && transaction.signatures);
    const shouldStripFeeTokenForSponsorship = (feePayer === true && (!hasSenderSignature || !feePayerSignature)) ||
    ...
    const signature = (() => {
        ...
        const sorted = typeof transaction.multisig === 'string'
            ? tempo_1.SignatureEnvelope.sortMultisigApprovals({
    ...
    if (signature && typeof transaction.feePayer === 'object') {
        ...
        const sender = transaction.from ??
            tempo_1.SignatureEnvelope.extractAddress({
