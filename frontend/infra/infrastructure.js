import { isAvailable, modeBadge, readableStatus, statusBadge } from "../js/utils.js";
import { emptyNotice, icon, pageHeader, sectionHeader } from "../js/components/common.js";

let wdkConnected = false;
let wdkAddress = "";
let wdkBalance = "—";

async function initWDK() {
  await import("../js/wallet/connector.js").then(m => m.initWDKWallet());
...
function updateWalletUI() {
  const walletNode = document.querySelector('.diagram-node--root');
  const flowStatus = document.querySelector('.infra-flow__path');
  
  if (wdkConnected && walletNode) {
    walletNode.classList.add('connected');
    walletNode.querySelector('span')?.remove();
    const addressShort = `${wdkAddress.substring(0, 6)}...${wdkAddress.substring(-4)}`;
    const addrSpan = document.createElement('span');
    addrSpan.textContent = addressShort;
    addrSpan.style.cssText = 'color: var(--ink); font-family: "IBM Plex Mono", monospace; font-size: 0.61rem;';
    walletNode.querySelector('div').appendChild(addrSpan);
  }
  
  // Update flow status text
  if (flowStatus) {
    flowStatus.textContent = wdkConnected ? 'WDK wallet connected' : 'Connect wallet to enable';
  }
  
  // Update card footers
  const cardFooters = document.querySelectorAll('.infra-card__footer span');
  if (cardFooters.length > 0) {
    cardFooters[0].textContent = wdkConnected ? 'WDK wallet' : 'Status source';
    cardFooters[1].textContent = wdkConnected ? 'backend' : 'fixture';
  }
}
